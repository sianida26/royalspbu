...
export default function ModalResetPassword(props: Props) {
    ...
    const handleResetPassword = () => {
        ...
        axios({
            ...
            data: {
                id: props.user.id,
                password: password, //security measure
            }
        })
        .then(() => { //handle success response
            enqueueSnackbar(`User ${props.user.username} berhasil direset password`,{variant: 'success'})
            cleanModal()
            props.onFinished()
        })
    ...
    return (
        <div style={{zIndex: zIndexes.modalBackdrop}} className={`tw-fixed tw-top-0 tw-left-0 tw-w-screen tw-h-screen tw-p-4 ${props.show ? 'tw-grid' : 'tw-hidden'} tw-place-items-center tw-bg-black tw-bg-opacity-75`}>
            <div className="tw-bg-white tw-rounded-lg tw-py-4 tw-px-8 tw-w-full tw-max-w-screen-sm tw-flex tw-flex-col tw-items-center">
                ...
                <h1 className="tw-text-center tw-font-bold tw-text-2xl">Yakin ingin reset password {props.user.name} ?</h1>
                <p className="tw-my-4 tw-text-gray-700">
                    Password yang telah direset akan diubah kembali menjadi <em className="tw-font-bold">{props.defaultPassword}</em>.
                </p>
