...
export default function ModalDeleteTank(props: Props) {
    ...
    React.useEffect(() => {
        if (!props.show || props.tank.isNotDefined()){
    ...
    const handleDeleteTank = () => {
        ...
        .then(() => { //handle success response
            enqueueSnackbar(`Tangki ${props.tank.name} berhasil dihapus`,{variant: 'warning'})
            cleanModal()
            props.onFinished()
        })
    ...
    return (
        <Backdrop show={props.show}>
            <div className="tw-bg-white tw-rounded-lg tw-py-4 tw-px-8 tw-w-full tw-max-w-screen-sm tw-flex tw-flex-col tw-items-center">
                ...
                <h1 className="tw-text-center tw-font-bold tw-text-2xl">Yakin ingin menghapus tangki {props.tank.name} ?</h1>
                <p className="tw-my-4 tw-text-gray-700">
                    Tangki yang telah dihapus <b>tidak dapat dikembalikan lagi</b>. Apakah Anda yakin?
                </p>
