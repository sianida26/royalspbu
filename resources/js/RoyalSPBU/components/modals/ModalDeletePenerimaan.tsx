...
export default function ModalDeletePenerimaan(props: Props) {
    ...
    React.useEffect(() => {
        if (!props.show || props.penerimaan.isNotDefined()){
    ...
    const handleDeletePump = () => {
        ...
        axios({
            ...
            data: {
                id: props.penerimaan.id,
                password: password,
            }
        })
        .then(() => { //handle success response
            enqueueSnackbar(`Penerimaan berhasil dihapus`,{variant: 'warning'})
            cleanModal()
            props.onFinished()
        })
    ...
    return (
        <Backdrop show={props.show}>
            <div className="tw-bg-white tw-rounded-lg tw-py-4 tw-px-8 tw-w-full tw-max-w-screen-sm tw-flex tw-flex-col tw-items-center">
                <i className="bi bi-exclamation-triangle-fill tw-text-red-500 tw-text-4xl tw-mt-4" />
                <h1 className="tw-text-center tw-font-bold tw-text-2xl">Yakin ingin menghapus penerimaan ini ?</h1>
                <p className="tw-my-4 tw-text-gray-700">
                    Item yang telah dihapus <b>tidak dapat dikembalikan lagi</b>. Apakah Anda yakin?
                </p>
